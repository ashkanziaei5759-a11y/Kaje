...
export default async function MenuItemPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  return (
    <>
      <div className="mb-4">
        <Link href="/menu-items" className="text-2xs text-ink-500 hover:text-saffron-400">
      ...
      <section className="card p-5">
        <div className="flex flex-wrap items-end justify-between gap-6">
          <div>
            ...
            <p className="mt-1 text-3xl font-bold tabular text-ink-50">
              {formatCurrency(totalCost, { symbol })}
            </p>
          </div>
          <div>
            <p className="label">قیمت فروش</p>
            <p className="mt-1 text-3xl font-bold tabular text-saffron-400">
          ...
          <div>
            ...
            <p className={`mt-1 text-3xl font-bold tabular ${
              costed.isUnprofitable ? 'text-pomegranate-400'
                : costed.isBelowMinimumMargin ? 'text-saffron-400' : 'text-pistachio-400'
            }`}>
        ...
        {costed.isBelowMinimumMargin && (
          <p className="mt-4 rounded-lg border border-saffron-400/30 bg-saffron-400/10 px-3 py-2 text-2xs leading-6 text-saffron-300">
      ...
      <div className="mt-4 grid lg:grid-cols-3 gap-4">
        ...
        <section className="card p-4">
          <h2 className="text-sm font-semibold text-ink-100">اجزای قیمت تمام‌شده</h2>
          <dl className="mt-3 space-y-2">
            ...
            <div className="flex items-center justify-between border-t border-ink-800 pt-2">
              <dt className="text-sm font-semibold text-ink-100">مجموع</dt>
              <dd className="text-sm font-bold tabular text-ink-50">
                {formatCurrency(totalCost, { symbol })}
              </dd>
            </div>
          </dl>
        ...
        {/* Price control */}
        <section className="card p-4">
          <h2 className="text-sm font-semibold text-ink-100">قیمت‌گذاری</h2>
          <dl className="mt-3 space-y-2 text-2xs">
            <div className="flex justify-between">
              <dt className="text-ink-400">پروفایل</dt>
              <dd className="text-ink-200">{profile.name}</dd>
            </div>
            <div className="flex justify-between">
              <dt className="text-ink-400">هدف حاشیه سود</dt>
              <dd className="tabular text-ink-200">{formatPercent(Number(profile.targetGrossMargin), { decimals: 0 })}</dd>
            </div>
            <div className="flex justify-between">
              <dt className="text-ink-400">قیمت پیشنهادی (خام)</dt>
              <dd className="tabular text-ink-400">{formatCurrency(costed.rawRecommendedPrice)}</dd>
            </div>
            <div className="flex justify-between">
              <dt className="text-ink-400">پس از گرد کردن</dt>
              <dd className="tabular text-saffron-400 font-semibold">
                {formatCurrency(costed.recommendedPrice, { symbol })}
              </dd>
            </div>
            <div className="flex justify-between">
              <dt className="text-ink-400">مالیات ({formatPercent(Number(profile.taxRate), { decimals: 0 })})</dt>
              <dd className="tabular text-ink-200">{formatCurrency(costed.taxAmount)}</dd>
            </div>
            <div className="flex justify-between">
              <dt className="text-ink-400">قیمت بدون مالیات</dt>
              <dd className="tabular text-ink-200">{formatCurrency(costed.priceExcludingTax)}</dd>
            </div>
          </dl>
        ...
        {/* Ratios */}
        <section className="card p-4">
          <h2 className="text-sm font-semibold text-ink-100">نسبت‌ها</h2>
              ...
              <ul className="space-y-1">
                {item.modifierGroups.map((g) => (
                  <li key={g.id} className="flex justify-between text-2xs">
                    <span className="text-ink-300">{g.modifier.namePersian}</span>
      ...
      <section className="mt-4">
        <h2 className="mb-3 text-sm font-semibold text-ink-100">
          ریز مواد مصرفی
          {item.recipe && (
            <span className="mr-2 font-normal text-2xs text-ink-500">
      ...
      <section className="mt-4 card p-4">
        <h2 className="text-sm font-semibold text-ink-100">فرمول‌های محاسبه</h2>
            ...
            <tbody className="divide-y divide-ink-850">
              {costed.formulas.map((formula, index) => (
                <tr key={index}>
                  ...
                  <td className="td font-mono text-2xs text-ink-400" dir="ltr">{formula.expression}</td>
                  <td className="td font-mono text-2xs text-ink-500" dir="ltr">{formula.substituted}</td>
                  <td className="td tabular text-ink-100" dir="ltr">
      ...
      {item.recipe && item.recipe.versions.length > 0 && (
        <section className="mt-4">
          <h2 className="mb-3 text-sm font-semibold text-ink-100">تاریخچه نسخه‌های دستور پخت</h2>
          <Table>
            <thead>
              <tr className="border-b border-ink-800">
            ...
            <tbody className="divide-y divide-ink-850">
              {item.recipe.versions.map((version) => (
                <tr key={version.id}>
                  ...
                  <td className="td text-ink-400">{formatJalali(version.createdAt)}</td>
                  <td className="td tabular">{formatCurrency(version.costPerPortion.toString(), { symbol })}</td>
                  <td className="td text-ink-400">{version.changeNote ?? '—'}</td>
                </tr>
              ))}
            </tbody>
          </Table>
...
function CostRow({
  ...
  return (
    <div className="flex items-center justify-between">
      <dt className={`text-2xs ${muted ? 'text-ink-500' : 'text-ink-400'}`}>{label}</dt>
      <dd className={`text-sm tabular ${muted ? 'text-ink-500' : 'text-ink-200'}`}>
  ...
  return (
    <div>
      <div className="flex items-center justify-between text-2xs">
        ...
        <span className={`tabular ${overTarget ? 'text-saffron-400' : 'text-ink-200'}`}>
          {formatPercent(value)}
        </span>
      </div>
      <div className="mt-1 h-1.5 overflow-hidden rounded-full bg-ink-850">
        <div
          className={`h-full rounded-full ${overTarget ? 'bg-saffron-400' : 'bg-ink-500'}`}
          style={{ width: `${width}%` }}
        />
      </div>
    </div>
...
function CostLineRows({
  ...
  return (
    <>
      <tr className={depth > 0 ? 'bg-ink-950/40' : ''}>
        <td className="td" style={{ paddingRight: `${1 + depth * 1.25}rem` }}>
          <span className={depth > 0 ? 'text-ink-400' : 'text-ink-100'}>
            {depth > 0 && <span className="text-ink-600 ml-1.5">└</span>}
            {line.namePersian}
          </span>
        ...
        <td className="td tabular text-ink-400">
          ...
          <span className="mr-1 text-ink-600">{line.unitCode}</span>
        </td>
        <td className="td tabular text-ink-400">
        ...
        <td className="td tabular text-ink-100">{formatCurrency(line.totalCost)}</td>
        <td className="td">
          <div className="flex items-center gap-2">
            <div className="h-1.5 w-12 overflow-hidden rounded-full bg-ink-850">
              <div className="h-full bg-ink-500" style={{ width: `${Math.min(100, share * 100)}%` }} />
            </div>
            <span className="tabular text-2xs text-ink-500">{formatPercent(share, { decimals: 0 })}</span>
          </div>
        </td>
      </tr>
